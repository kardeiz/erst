...
fn template_derive_inner(input: syn::DeriveInput) -> Result<TokenStream, Box<std::error::Error>> {
    ...
    for pair in input.attrs.iter()
    	.flat_map(|x| x.parse_meta() )
    	.filter(|x| x.name() == "template" )
    	.filter_map(|x| match x { syn::Meta::List(ml) => Some(ml), _ => None })
    	.flat_map(|x| x.nested )
    	.filter_map(|x| match x { syn::NestedMeta::Meta(m) => Some(m), _ => None })
    	.filter_map(|x| match x { syn::Meta::NameValue(nv) => Some(nv), _ => None }) {
    ...
    let path = std::env::var("ERST_TEMPLATES_DIR").map(PathBuf::from)
    	.or_else(|_| std::env::var("CARGO_MANIFEST_DIR")
    		.map(|x| PathBuf::from(x).join("templates") ))?
    ...
    let body_marker = syn::Ident::new(&format!("__ERST_BODY_MARKER_{}", &name), proc_macro2::Span::call_site());
...
fn parse(template: &str, type_: &str) -> Result<String, Box<std::error::Error>> {
	...
	use erst_shared::{exp::Parser as _, parser::{Rule, ErstParser}};
	...
	for pair in pairs {
		match pair.as_rule() {
			...
			Rule::expr => {
				match type_ {
					"html" => {
						buffer.push_str(&format!("write!(__erst_buffer, \"{{}}\", erst::Html({}))?;", pair.into_inner().as_str()));
					},
					_ => {
						buffer.push_str(&format!("write!(__erst_buffer, \"{{}}\", {})?;", pair.into_inner().as_str()));
					}
				}
